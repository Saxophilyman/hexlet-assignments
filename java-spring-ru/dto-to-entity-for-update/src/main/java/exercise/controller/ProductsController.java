...
public class ProductsController {
    ...
    public ProductDTO update(@RequestBody ProductUpdateDTO productUpdateDTO, @PathVariable long id){
        var product = productRepository.findById(id).orElseThrow(()->new ResourceNotFoundException("Product with id " + productUpdateDTO.getId() + "Not found"));
        toEntityForUpdate(productUpdateDTO,product);
        productRepository.save(product);
        var productDTO = toDTO(product);
        return productDTO;
    }
    ...
    private Product toEntityForUpdate(ProductUpdateDTO productUpdateDTO, Product product){
